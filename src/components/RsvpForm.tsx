...
const RsvpForm: React.FC = () => {
  ...
  const handleRsvpAnswerChange = (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const submitForm = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (name.length === 0 || email.length === 0 || rsvpAnswer.length === 0) return;
      ...
      <form className='flow' onSubmit={submitForm}>
        ...
        <div className='form-field'>
          <fieldset>
            ...
            <div className="radio-wrapper">
              <input type="radio" id="rsvp-yes" name="rsvp-answer" value="rsvp-yes" onChange={handleRsvpAnswerChange} required/>
              <label htmlFor="rsvp-yes">Yes</label>

              <input type="radio" id="rsvp-no" name="rsvp-answer" value="rsvp-no" onChange={handleRsvpAnswerChange} required/>
        ...
        </div>
        {
          status === 'SUCCESS' ?
            ...
            <button type="submit" aria-label="submit contact form" className="button" disabled={name.length === 0 || email.length === 0 || rsvpAnswer.length === 0}>
